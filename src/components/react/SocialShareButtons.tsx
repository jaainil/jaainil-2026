export function SocialShareButtons({ articleTitle, articleUrl }: { articleTitle: string; articleUrl: string }) {
  ...
  const handleMouseEnter = (e: React.MouseEvent<HTMLElement>) => {
    ...
    target.style.backgroundColor = 'var(--color-hover)';
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-[11px] font-mono uppercase tracking-widest hidden sm:block" style={{ color: 'var(--color-text-muted)' }}>
        Share:
      </span>
      {shareLinks.map((link) => (
        link.action === 'copy' ? (
